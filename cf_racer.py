...
def ratings_scraper(handle):
    ...
    if resp.status_code == 200:
        ...
        for contest in result:
            ...
            contest_name = contest_name.replace("Round ", "").replace("Codeforces", "CF")
            contest_date = dt.fromtimestamp(contest['ratingUpdateTimeSeconds']).date()
            contest_rating = contest['newRating']
            CONTESTS[contest_date] = contest_name
            ratings_data[contest_date] = contest_rating
        
        REAL_NAME[handle] = handle  # Use handle as the name
        return ratings_data
    
    else:
        print(f"Error: Could not connect to Codeforces API for handle {handle}")
